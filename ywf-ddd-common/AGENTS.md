...
本树=公共框架（8 个 opt-in 模块）。进入本目录先读根 `AGENTS.md` 九条；分层/命名/禁令法条在 `.agents/rules/02-04`，此处不复述。
...
- **构件身份**：工具库 vs 定型装配判据见 rules/04「Common 模块约束」（exclusions 卫生集中法同条）；本树 javadoc 示例代码位同守教学中立（虚构教例用 Payment 家族，禁 order/product 业务词）。
