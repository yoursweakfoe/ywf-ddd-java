 ...
 *
 * <p>枚举值与 PostgreSQL 类型名称一一对应，用于 {@link AbstractArrayTypeHandler} 创建 SQL Array 时指定类型。
 *
 ...
 * StringArrayTypeHandler}）。 这些处理器通过 {@code @MappedTypes(Xxx[].class)} 注解向 MyBatis 注册， 使得 PO
 * 实体类中的数组字段可以自动匹配到正确的处理器，无需显式指定。
 *
