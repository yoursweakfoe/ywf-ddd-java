 ...
 *
 * <p>位于 {@code adapter/event/consumer/}。实现类为普通 {@code @Component}（MQ 监听注解的
 ...
 *   <tr><td>{@code IntegrationEventSubscriber}（本接口）</td><td>adapter/event/consumer</td><td>集成事件入（MQ 接收）</td></tr>
 *   <tr><td>{@code IntegrationEventPublisher}</td><td>application/event/publisher</td><td>集成事件出（跨进程，本角色的对偶）</td></tr>
 *   <tr><td>{@code DomainEventSubscriber}</td><td>application/event/listener</td><td>领域事件入（进程内）</td></tr>
 * </table>
 *
