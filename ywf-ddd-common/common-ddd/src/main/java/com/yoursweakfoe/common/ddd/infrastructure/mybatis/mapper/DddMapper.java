 ...
 * <ul>
 *   <li><b>insert</b> — 枚举全部业务列（含 created_at / updated_at 审计列，由
 ...
 *       （updated_at = #{updatedAt}）。逻辑删除聚合必须带 {@code AND deleted = false} 过滤。</li>
 ...
 *       {@code UPDATE {table} SET deleted = true, updated_at = #{now}[, updated_by = #{updatedBy}]
...
public interface DddMapper<PO> {
     ...
     *
     * @param id        持久化主键
     * @param now       本次删除时刻（基类经注入 Clock 生成，供逻辑删除 SET 子句刷新 updated_at）
