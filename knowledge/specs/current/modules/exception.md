...
> **机器对账**：本卷在 check-docs C1（`{agg}` 模板实例化）/ C3（符号解析）/ C4（教学中立）扫描面内。开册法案：`2026-09-framework-codification`；§4 全链路条款由 `2026-09-howto-codification` 增册（源 = `how-to/error-handling.md` 入法）；统一用法归卷：`2026-09-usage-consolidation`（§5 全链路形状即归卷执行）。
...
| EV-9 | 前端渲染契约：仅 **BusinessException 通道**的 `detail` 可作 i18n key（`t(key, params)` 渲染）；校验族/技术异常/500 通道 `detail` 为固定泛化文案、禁作 key；`params` 仅 BusinessException 非空参数时出现（可选）；`fieldErrors` 仅 400 族；前端**不解析** `title` / `status`（仅日志监控） | 原篇「前端对接说明」入法；形状 §5.7 | 评审项 |
| EV-10 | 异常不经过 contract 层：全链路 contract 零异常件，错误语义只靠 messageKey 承载 | 原篇完整文件清单入法（contract 行=「无」）；§5.1 表 | 评审项 |

## §5 全链路形状（统一用法唯一样本）

> 教例家族：**虚构**（`payment` / `inventory` 聚合，sample 未实现）——本节只教形状；真实前缀指针见 §5.3 末（真实例）。异常从领域层到前端的设计叙事与 key 登记簿见 docs 设计卡（`../../../docs/how-to/error-handling.md`）。
...
全仓 key 清单**不**登记在本卷——登记簿（ledger）留在 docs 设计卡（EV-2 载明的宽松件职责）。
...
完整映射表（含 400 族 / 404 / 405 / 415 等框架客户端异常全部行）canonical 见 [../../docs/reference/api/common-exception.md](../../../docs/reference/api/common-exception.md) §2 与 `GlobalRestExceptionHandler` 类 javadoc「框架客户端异常显式映射表」，本卷不复制。核心通道（与 §5.1 传播链一致）：
...
### 5.6 响应形状（RFC 9457 问题详情，EV-8 的体面）
...
> 技术类异常的 `detail` 为稳定泛化文案（防内部实体 ID / SQL 片段外泄）；原始冲突消息只进服务端日志。识别乐观锁冲突请依赖**异常类型**（`OptimisticLockConflictException`），见 [../../docs/how-to/optimistic-lock-retry.md](../../../docs/how-to/optimistic-lock-retry.md)。
...
> **安全注意（EV-3）**：**禁止**在 `params` 中放入敏感信息（密码、Token、内部 ID 映射表、SQL 语句等）。
