...
> **机器对账**：本卷在 check-docs C1（`{agg}` 模板实例化）/ C3（符号解析）/ C4（教学中立）扫描面内。开册法案：2026-09 框架成典案；§4 全链路条款由 2026-09 设计卡降格案 增册（源 = `how-to/error-handling.md` 入法）；统一用法归卷：2026-09 用法归卷案（§5 全链路形状即归卷执行）。
