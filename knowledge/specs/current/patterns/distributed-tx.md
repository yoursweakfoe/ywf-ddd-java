...
| # | SHALL | 取证 | 背书 |
|---|---|---|---|
| DT-1 | 边界选择按铁序执行。同一数据源的多聚合一致性**必须**用本地 `@Transactional`；仅跨数据源的一致性场景方可启用 `@GlobalTransactional`，因为 Seata AT 有全局锁开销 | 设计卡同题篇「能用本地事务就不用分布式事务」原则入法；`modules/cloud.md` 互指 | — |
| DT-2 | 启用 Seata 时，东西向 HTTP 调用**必须**双端透传 XID。发起端拦截器把 `RootContext.getXID()` 写入约定 header；接收端 filter 读取并 bind/unbind。缺一环即全局事务断链 | 设计卡同题篇 XID 透传节入法 | 模板见宽松件 |
| DT-3 | 业务数据源必须经 Seata 自动代理（DataSource proxy）方可参与 AT。禁止手工注册旁路数据源 | 设计卡同题篇自动代理节；`modules/cloud.md` 条件装配条款 | — |
| DT-4 | 最终一致可接受的场景（通知、日志）禁止拉入分布式事务，改走异步消息 + 重试 | 设计卡同题篇边界表第三行入法 | — |
| DT-5 | `@GlobalTransactional` 标注在**发起方**（TC 协调入口）Handler 方法。跨服务调用与本地写在同一方法体内完成，模板序为先远程扣减、后本地落库。任一分支失败 → TC 通知所有分支回滚（undo_log 逆向补偿） | 本卷 §2.4 形状；设计卡同题篇「标注在发起方」要点入法 | — |
...
| DT-7 | XID 透传双端组件由业务侧按本卷模板手写。框架（common-cloud）**不内置**透传组件。Spring Cloud Alibaba 路线不适用，本项目未引入 | 设计卡同题篇「框架不内置透传组件」注记入法 | — |
...
> 真实例对照：示例应用即为此形态，两聚合同服务同数据源。落位 `sample-application/.../application/order/handler/command/PlaceOrderHandler.java`（真实例映射位，`@Transactional` 本地事务、无 Seata）。
