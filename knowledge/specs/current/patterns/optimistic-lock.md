...
> 三分通道围栏（OL-7）：本重试器只 catch `OptimisticLockConflictException`（UPDATE 版本冲突）；「实体消失」的普通 `IllegalStateException` 与其他 `SilentWriteLossException`（INSERT/DELETE 0 行写丢失，500+ERROR）**同样直接上抛**——后两者重试无意义，必须让告警吵醒人。

### 2.5 注意事项表（用法侧规范，随形状入卷）
...
| 条款/环节 | 状态 | 位置 |
|---|---|---|
| OL-1~4 | ✅ | 框架通道与示例聚合行为均有测试实证在册（原登记句全文保留） |
...
> **落地状态注记（真实例）**：重试模式已在示例应用 **PlaceOrder 链路真实落地**——`sample-service-server` 的 `RetryablePlaceOrderHandler`（包装 `PlaceOrderHandler`，由 `OrderAppService` 注入使用），含 4 个单测覆盖重试成功 / 非冲突穿透 / 耗尽上抛；§2.4 为虚构 payment 聚合的通用教学模板，结构与真实实现一致。
