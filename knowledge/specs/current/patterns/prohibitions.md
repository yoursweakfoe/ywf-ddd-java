...
- **允许（且应当）**承载 HTTP 映射 + 文档 + 校验注解，判据是重契约单一事实源、旧案卷判例：`@RequestMapping` 系 / `@Tag` `@Operation` `@Schema` / `@NotNull` `@Valid`。均为注解级依赖，映射经 ControllerImpl 继承承载 → [CC-7](coding-conventions.md)
...
- 禁止 `LocalDateTime` / `ZonedDateTime` 作持久化时间类型，统一 `OffsetDateTime`（旧案卷判例）。前者写入依赖会话时区、读 `timestamptz` 抛异常；后者 pgjdbc 双向抛异常 → [CC-5](coding-conventions.md)
- 禁止生产代码使用 `synchronized` 块/方法，虚拟线程会 pinning。互斥用 `ReentrantLock`，AGENTS 九条 8；项目启用 JDK21 虚拟线程。ThreadLocal 正常，身份上下文由 Security 链管理，业务代码勿手工清理

## §8 通用禁止

- 禁止 Mediator 模式。Handler 与 CQE 1:1，无中间路由，这是显式依赖教义
- 禁止在核心代码（`src/` / sample 树）嵌入 AI 工具专属指令。未入围扩展点按需引入，见 theory-map；Specification 为已采纳最小接口，非禁令对象

## §9 Common 模块约束（构件身份三分法 · 单一事实源）

审 common 依赖先查登记表，三种判据按身份分叉：

- **工具库**：依赖 = 本包编译所需。判据是最小化，超出即裁剪。
- **定型装配**（opinionated starter）：定性是"我们做的是脚手架封装，不是库封装"。依赖 = 使用方注定继承的**命运清单**。教义前提：所有采用方服务都是单 jar 全套四层，装配替使用方预先决策整条技术栈。
- **试验场**（`common-packages-integration-test`）：不发布、无消费方。依赖面 = 全 common + 任意测试栈，**无最小化质证义务**，污染止于自身。铁律反在产物侧：`src/main` 永空置，出现产码即违规，一切代码是测试；`maven.deploy.skip` 在位；真库基座只准真 PG（TC-9 框架轨/TC-10）。
...
**exclusions 卫生集中制**（两种身份一体适用）：依赖排除只写在策略文件，即 `ywf-ddd-common/pom.xml` 与 `sample-application/pom.xml` 的 `<dependencyManagement>`；子 pom 声明处零 exclusions。判据来自 Maven 语义：无局部清单时自动继承 managed exclusions，而任何局部 `<exclusions>` 块**整体替换**而非合并 managed 清单——在声明处写一条排除，就静默拆掉了全部集中卫生。判例：dynamic-datasource 的 Oracle UCP/ojdbc 条目与 seata starter 的 fastjson/druid/dubbo-filter-seata 条目。`<optional>/<scope>` 留在声明处，那是消费姿态，不是卫生政策。
...
| PB-G4 | 所有提交、分支管理、合并由人工完成 | 此前全部法案均如此落地，即判例 |
| PB-G5 | `.gitignore` / `.gitattributes` 允许 Agent 修改，属项目配置，非提交树 | — |

## §11 生效登记

全卷 ✅。各条 ArchUnit/测试/javadoc 背书签注于条款行。§9 登记表随模块增废经法案更新——本表即模块身份唯一登记处，javadoc 自宣言为第二道镜像。
