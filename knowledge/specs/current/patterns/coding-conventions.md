...
> **身份**：本卷是横切编码规范（命名 / 结构映射 / Lombok / 时间 / 安全上下文取用）的唯一权威；违反本卷=修代码，修卷走 `../../changes/`。前身 `.agents/rules/03` 升格入典（法案 2026-10 法条归典案——规范出工作台、入法典）。docs 侧复述均降为指针。
> **机器对账**：C1/C3/C4 扫本卷。
...
| CC-5 | 时间律：持久化与领域时间一律 `OffsetDateTime`；当前时间经注入 `Clock`，禁无参 `now()`；「同一瞬时」比较用 `isEqual()`/`timeLineOrder()`（`equals` 含偏移） | 旧案 判例；AGENTS 九条 8 | 测试可注入验证 |
...
| CC-7 | contract 模块内容白名单：仅 Controller 契约接口 + CQE + CO + 枚举；仅依赖 common-contract 标记接口；东西向复用同一契约（一期 RestClient 静态直连，Feign 经 common-cloud opt-in） | 旧案 重契约判例；[contract 卷](../modules/contract.md) | ArchUnit |
...
| CC-9 | 分页契约细则：`PageableQuery` 实现于 Query record；页码自 **1**；`pageNum`/`pageSize` 须显式传入（缺参→0→`@Min(1)`→400，无默认注入）；上限 `MAX_PAGE_SIZE`；`safe*()` 为执行侧第二道防线；`@Valid` 标于契约接口方法参数 | contract 模块卷 + `PageableQuery` javadoc；binding 沿革 = 历元一分页参数案（已清册归零） | 400 三通道测试 |
