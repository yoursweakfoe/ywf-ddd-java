...
> **身份**：本卷是 Portal/Gateway（外部系统 ACL）统一用法的唯一权威。全套规范形状只在本卷，全仓其他文档不得复写形状。违反本卷 = 修代码；修卷走 `../../../changes/`。docs 同题篇（`../../../../docs/how-to/gateway.md`）是设计卡，只讲选型与边界叙事，零形状代码；与本卷冲突时以本卷为准。
> **机器对账**：C1/C3/C4 扫本卷。教例家族为 Payment + 支付宝渠道（虚构教例，sample 未实现）。

## §1 条款

| # | SHALL | 取证 | 背书 |
|---|---|---|---|
| GW-1 | 外部能力在 domain 侧以 Portal 接口表达。出入参一律用领域对象或值对象，禁止出现外部 SDK 类型或传输格式 | 框架 `Portal` 标记接口，common-ddd 已备，见下；依赖倒置原则见 AGENTS 九条 5 | C3 |
...
| GW-4 | 一个 Portal 只表达一类外部能力。禁止「上帝 Gateway」聚合不相关外部系统 | 设计卡同题篇职责边界结论入法 | — |
| GW-5 | Portal 接口名以 `Portal` 结尾，且 `extends Portal`。`Portal` 是 common-ddd 的空标记接口，标记「外部能力抽象」身份。接口定义在 `domain/{agg}/portal/`，该包只准入接口 | 本卷 §2.2 形状；设计卡同题篇 §1 要点入法 | — |
| GW-6 | Portal 的返回值对象是值对象，落位 `domain/{agg}/model/`。聚合根、实体、值对象、枚举统一住 model/；跨聚合可放 `domain/shared/model/`。**不放 portal/ 包** | 本卷 §2.2 形状 + §2.5 命名表；directory-structure/server/domain.md「目录职责」表 | — |
...
| GW-8 | 使用方（Handler 或 Domain Service）注入 Portal 接口而非 Gateway 实现。事务内调用 Portal 时，事务边界仍在 Handler 的 `@Transactional` | 本卷 §2.4 形状：依赖 Domain 接口而非 Gateway 实现，事务边界仍在 Handler | — |
| GW-9 | Portal 接口所在的 domain 包零框架运行时依赖。common-ddd 纯标记接口豁免：`Portal` / `ValueObject` | AGENTS 九条 1 stereotype 豁免；本卷 §2.2 形状 | — |

## §2 规范形状（统一用法唯一样本）

> 本章是**虚构教例**：`payment` 聚合 + 支付宝渠道。示例应用刻意不演示 Portal/Gateway，sample 未实现，状态见 §3 注记。教学代码中的包路径、类名均为虚构落位。设计目标：Domain 层定义"我需要什么"（Portal 接口），Infrastructure 层决定"谁提供、怎么调"（Gateway 实现 + ACL 翻译）。切换支付渠道只需新增 Gateway 实现，Domain 零修改。
...
> 只做 `findById → 行为 → update → toDTO`，未注入任何 Portal。本节展示接入第三方
> 扣款时的正确挂接形态：依赖 Domain 接口而非 Gateway 实现，事务边界仍在 Handler（GW-8）。
...
> Repository → persistence 与 Portal → gateway 是「Domain 定义接口、Infra 实现」的对偶结构。两者的接口/实现包路径、操作对象、语义与翻译方式，对照表 canonical 收录于 `docs/explanation/infrastructure.md` 的 persistence / gateway 章节，本卷不复制。Portal/Gateway 的**代码形状**——接口定义、Gateway 实现、ACL 翻译、命名对偶——canonical 在本卷。
...
| 业务侧实现 | ⛔ sample 未实现 | 示例应用保持最小闭环，无任何 Gateway 业务实现，真实例核证见本节末注记。本卷条款对将来落地件生效 |
