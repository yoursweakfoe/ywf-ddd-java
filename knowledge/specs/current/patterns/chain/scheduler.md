...
| # | SHALL | 取证 | 背书 |
|---|---|---|---|
| SC-1 | 定时任务入口置于 `adapter/task/scheduler/`，实现 `ScheduledAdapter` 标记接口。R14a 管位置，R14b 管标记 | 框架 `ScheduledAdapter`（common-ddd 在库）；ArchUnit | C3 |
| SC-2 | Scheduler 方法体纯透传，只委托 AppService/Handler。禁止在调度类内写业务分支；与 Adapter 纯透传同规 | AGENTS 九条 2；WC-4 互指 | ArchUnit |
| SC-3 | 时间判断一律使用注入的 `Clock`，禁止直调 `OffsetDateTime.now()`。目的：统一时间源、可测试 | AGENTS 九条 8；`modules/ddd.md` Clock 条款 | 测试可注入验证 |
...
> 本章代码块是 {Agg} 通式教学模板，业务无关、虚构教例。案例是「自动完成超时未确认订单」式的时间驱动入口。在示例应用的具体落位名以 §3 生效登记为准，属真实例映射。
...
adapter 层框架内置两类 driving adapter，即协议适配入口。Scheduler 是其中由时间驱动的一类：
...
> MQ 消费类入口按同一包位置与角色惯例落位，与 rest / task 同构，同样纯透传。
...
- `updateDomainBatch` 属于 `MybatisPersistence` 基类，是 `RepositoryImpl` 继承后暴露的基行为，不是 domain `Repository` 五方法生命周期契约的成员。批量原子性由 Handler 的 `@Transactional` 保证，框架方法本身不标注。
- `findShippedBefore` 是决策型读，按业务命名追加在 `{Agg}Repository` 子接口上，由具名 Mapper 方法实现（契约见 `how-to/new-aggregate.md` ⑭/⑰）。
- 批量体量：单事务逐条循环，超大批量由调用方自行分片，每批 ≤500 条（消费契约见法卷 [batch-write.md](batch-write.md) BW-5）。
...
对 `ScheduledAdapter` 标记与 R14a/R14b 的影响是零。规则只认包位置和标记接口，不认触发注解。
...
> 幂等责任（SC-8）：无论哪种模式，平台的失败重试都不是可重入豁免。Handler/聚合侧的状态机守卫与幂等语义照常承担。

### 2.9 下游协调注记（SC-7 形状说明）

`{agg}.deliver()` 等聚合行为方法只做状态变迁与校验。需要联动其他聚合时，由 Handler / DomainService 同事务直调。原因：时间只是又一种触发源，下游与各入口完全一致。
...
| sample 落地 | ⛔ | 真实例映射登记于本节落位清单；调度器示例件未全量落地，以本卷 §2 模板为准。多实例分布式锁义务见 SC-4 |
