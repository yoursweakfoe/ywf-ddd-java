...
> **机器对账**：C1/C3/C4 扫本卷；教例家族 `{Agg}`/`{Action}` 通式 + 虚构 Fixture（`Test{Agg}s`），真实例只准出现在带「真实例」标记的指针位（§2.8）。开册法案：2026-09 设计卡降格案；统一用法归卷：2026-09 用法归卷案。

## §1 条款

| # | SHALL | 取证 | 背书 |
|---|---|---|---|
| TC-1 | 测试四分型强制：Handler 单测（Mockito，零 Spring 容器）/ Domain 纯 JUnit / Converter 往返（PO↔domain 等价）/ 集成（test profile + 真 PG 测试库，TC-9）；容器测试只准走 test profile；**真库测试基座全仓唯一 = PostgreSQL，H2 已于 2026-09 全仓退役（含 common，TC-10）** | `modules/test.md`（common-test 场景 2/3）；原篇四型节入法 | mvn 全绿 |
...
| TC-5 | 演示契约（2026-09 改卷）：全仓测试零网络出口；运行前置 = ywf-infra postgres 环节 + db-migration 对 `ddd_sample_application_test` 建形（一条命令，`db-migration/README.md`）；common 试验场库 `ddd_framework_test` 由 `PgTestSupport` 自建自清，零附加前置（TC-9/TC-10） | 示例树宪法（`sample-application/AGENTS.md`）在册承诺随本案同步改写 | CI 复跑 |
| TC-9 | 真库测试基座与隔离双轨：**服务轨** test profile 直连 PG 测试库，库形状权威 = db-migration（测试不建表）；容器测试默认 `@Transactional` 回滚，真 HTTP 链教例（独立事务提交、回滚不可覆盖）SHALL 类级 `@Sql TRUNCATE … RESTART IDENTITY` 清场。**框架轨** common fixture 形状非服务形状、不入库 db-migration，形状权威 = `PgTestSupport` 三段生命周期（自建→每 JVM 复位→shutdown 清空，schema `integration_test` / `integration_test_vacant`）；fixture 表名 SHALL 不带 schema 限定、可见性由数据源 `currentSchema` 裁决，「空库」教例 = 真空 schema | `integration/RestEndpointIntegrationTest` / `OptimisticLockConcurrencyTest` 类级 @Sql；`common.integration.pg.PgTestSupport` + `MybatisPersistenceTest.dynamicRouting_secondSourceIsBare_masterHasSchema` | mvn 全绿（真 PG ×2 轮 + 轮间零残留实证） |
| TC-10 | common 诸包测试集中法（2026-09 开卷）：`ywf-ddd-common` 下一切测试（含单模块独立单测）只住 `common-packages-integration-test/src/test/`，各 library 模块 `src/test/` 不得存在、POM 不得声明测试栈；迁入卷 SHALL 保持与被测类全同包名（classpath split package 合法，工程不用 JPMS），**禁止**为测试搬迁扩大产码可见性；试验场 `src/main` 永空置、`maven.deploy.skip`（身份登记 → 禁令卷 §9「试验场」） | `common-packages-integration-test/pom.xml` + 模块树（55 卷迁居实证）；`MybatisPersistenceTest`（原包保白盒） | check-docs C2 + mvn 全绿 |
| TC-6 | 单元类覆盖下限：正常路径 + ≥2 异常路径；Handler 单测断言「委托链」（load→行为→save→toDTO），**不**重复断言领域规则——规则归 B 型测试 | 原篇 A 节要点 + 验收清单入法；形状 §2.1 | 评审项 |
| TC-7 | 测试文件位置镜像产码路径（辖域=服务树；common 试验场按 TC-10 保原包镜像，不受本条 `integration/` 集中句约束）：A `application/{agg}/handler/command/`（读侧镜像 `handler/query/`）、B `domain/{agg}/model/`、C `infrastructure/persistence/{ds}/{agg}/converter/`；集成测试统一住 `integration/` | 原篇 A–D 各节「位置」行入法；形状 §2.1–§2.4 | 评审项 |
...
位置：`integration/{Feature}IntegrationTest.java`。test profile 直连真 PG 测试库（运行前置见 TC-5；库形状权威 = db-migration，测试代码不建表、不管 DDL）。真 PG 真方言真 SQL，无兼容模式转译层（入口见 [../../docs/tutorials/quickstart.md](../../../docs/tutorials/quickstart.md) 路线 A）。框架轨（common 试验场，TC-10）同走真 PG，唯形状权威换 `PgTestSupport` 三段生命周期、卷位置保原包镜像。
