...
→ [directory-structure/server/adapter.md](../reference/structure.md)

> 完整代码示例 → [cookbook/write-path.md](../how-to/write-path.md)（Controller 在写路径中的位置）
...
| Scheduler | `XxxScheduler` | 定时任务入口 → 透传 AppService（实现 `ScheduledAdapter` 标记，规则 R14a/R14b；模板见 [cookbook/scheduled-task.md](../how-to/scheduled-task.md)） |
