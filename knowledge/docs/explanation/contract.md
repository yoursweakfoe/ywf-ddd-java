...
- **CQRS 标记纯类型**：Command / Query 只作请求归类标记，不携带返回类型泛型——契约在外、处理器在内，把返回类型焊进标记等于把内部实现类型钉进对外契约；返回类型归处理侧方法签名承载，内外各自演进
...
这里曾有过另一种形状：**轻契约**——契约接口只含纯类型，HTTP 映射全部留在服务端 Controller，契约层自称「零框架依赖」。这一立场后来被反转，裁决理由一句话：**「协议无关的轻契约」是伪命题——每个协议都需要自己的契约表述**（HTTP 用映射注解，gRPC 用 protobuf 定义）。剥离协议信息后交付的只是半份契约：消费方看得见方法签名，看不见路径，路径沦为散落在消费方、服务方、网关三方之间的口头约定。

映射上契约之后同时解决三件事：

- **契约完整性**：类型 + 语义 + 路径一体分发，消费方拿接口即拿全貌，直接构造强类型调用
- **路径归属天然解决**：路径声明在契约接口上，三方对账有了事实源（即下文「契约访问边界」的路径命名空间约定所依托的那份归属）
- **文档注解有锚**：能力描述与 HTTP 映射挂在同一方法上，不再两处漂移

代价是显式接受的：契约模块引入 `spring-web`——「零框架依赖」在此精确化为**「零运行时依赖」**：契约的框架相关依赖一律停在注解层，不拉起容器，消费方即使不在 Web 运行时里也不受牵连。若将来引入第二协议（如内部 gRPC 调用），解法不是迁移现有 HTTP 契约，而是以该协议自身的契约表述另立一份独立重契约，与 HTTP 契约并存、互不影响——换协议是新增契约，不是改写契约。
