...
| 模式法卷摆架（十摊入路径） | 2026-09 pattern-taxonomy 案。法卷目录单一轴=设计关切（concern-based），拒任务轴（路由已有 how-to/skills 唯一住所）与层轴（链卷跨层证伪）；摊卡理解文本具法条身份、空穴摊有牌无肉、条款搬家原号随身、新铸编号=摊名式。分架先例谱系：Fowler 目录十组、Evans Reference 六节、微软云模式 8 类、PMD 八类、AWS 六柱、ISO 42010 视点=关切簇；页眉标签先例=GoF 双轴表。全文 → [pattern-taxonomy.md](pattern-taxonomy.md) |
