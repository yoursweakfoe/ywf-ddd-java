...
`knowledge/docs/reference/api/` 8 篇文档各自独立编号（ADR-0001 曾被 8 个模块重复定义），本表为全仓 ADR 的**唯一对照总索引**。md 层引用一律使用限定名 `模块 §ADR-000N`；废弃编号**保留空置、不重排、不迁移**。共 32 个编号位（29 条正文 + 3 个空置位）。
