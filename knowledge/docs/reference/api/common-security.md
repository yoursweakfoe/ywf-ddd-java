...
零信任身份 —— 每个服务自验 JWT（OAuth2 资源服务器模式），身份不传播，验签方案可插拔，claim 字段不写死。

> 本文分两段：§1–4 面向使用者（怎么用），§5–7 面向设计者（为什么这么设计）。

## 1. 定位与边界

在零信任 / 防御纵深架构下，为微服务提供 JWT 身份验签与解析能力。每个服务本身就是 OAuth2 资源服务器，自行验签 JWT，**不信任网络里的任何身份 Header**。

> 本模块只抽象机制，不固化策略。验签可插拔：`JwtDecoder` 可换、可按算法分发。身份字段不投影成固定结构，按名字自取。非标 claim、多密钥方案、字段数量不定这些现实情况，都不写死进框架。

## 2. 核心能力

### 包结构

模块根包 `com.yoursweakfoe.common.security` 只放自动装配，与其它 common 模块的约定一致；其余内容按话题分子包：
...
principal 是 Spring Security 原生 `Jwt`，**它本身就是 claims 的全量映射表**：`getClaims()` 返回 `Map<String,Object>`。字段名和数量都不设限，框架不做任何固定结构投影。
...
字段增减不影响既有代码：没有 `uname` 这个 claim 时，`getString("uname")` 返回 null，不抛异常；多了部门字段，`getString("department")` 照常按名字读取。

### 多验签方案（不同来源不同算法）

`DelegatingJwtDecoder` 按 JOSE 头 `alg` 分发到对应的 `JwtDecoder`：
...
| Bean | 条件 | 说明 |
|------|------|------|
| `JwtAuthenticationConverter` | `@ConditionalOnMissingBean` | 角色 claim 转 `ROLE_*` 权限，claim 名与权限前缀可配，principal 保持原生 `Jwt`；消费方自定义了该 Bean，框架就不注册 |
| 资源服务器 `SecurityFilterChain` | `@ConditionalOnMissingBean(SecurityFilterChain)` | `oauth2ResourceServer().jwt()` + CSRF 关闭 + 无状态 + permit-all |
| `@EnableWebSecurity` / `@EnableMethodSecurity` | 受 `ywf.security.enabled` 开关控制，缺省启用 | 启用 `@PreAuthorize` / `@Secured`；`enabled=false` 时整个配置类不激活 |

## 3. 使用方式

> **严格规范在法卷**：本节正文已入法，见 [../../../specs/current/modules/security.md](../../../specs/current/modules/security.md)。条款、代码形状、禁则以法卷为准，本文只留宽松指引。
...
- **零信任 / 防御纵深**：服务自验 JWT，不信任网络身份 Header；网关只是 PEP，转发 JWT 而不是转发身份断言
- **机制与字段分离**：框架抽象的是验签、建身份、无状态链、方法级鉴权；密钥方案、字段命名、字段数量全部可插拔或按需读取
- **身份即 Jwt，不投影**：principal 是原生 `Jwt`，即 claims 全量映射表，不投影成固定 record；字段数量不定，投影不了
- **fail-closed**：坏 token 直接 401，绝不静默放行；旧框架的 `catch (Exception ignored)` 是反例
- **边界 permit-all + 方法级鉴权**：路由级鉴权在网关；服务用 `@PreAuthorize` 做细粒度鉴权
...
| 项 | 说明 |
|---|---|
| 边界：JWT 签发 / 刷新 / 登出 | 由独立认证服务处理，即 IdP；**服务侧不提供签发能力**。旧框架用 `generateToken` 把签发下发到服务，是反例 |
| 边界：路由级鉴权 | 由网关（PEP）处理 |
...
| 风险：HMAC 共享密钥 | 对称密钥意味着验签方 = 签收方，任一服务被攻破即可伪造 token；长远可迁非对称方案，即 JWKS |
