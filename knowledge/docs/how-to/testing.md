...
> **本篇 = 设计卡（宽松件）**：只回答"该测什么、四类怎么分"。四型模板、Fixture、命名、验收终板与全部条款见法卷 → [../../specs/current/patterns/testing-conformance.md](../../specs/current/patterns/testing-conformance.md)。条款冲突以法卷为准，见归属法卷 §2。技能 `.agents/skills/new-test` 的模板指向随本次归卷重定向至法卷，技能文件本次不动。
