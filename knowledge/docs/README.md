...
> 本区是地图：描述代码。代码变了这里必须跟着变，与代码不符就是文档 bug。守则全文在 `knowledge/specs/current/patterns/attribution-law.md`。
