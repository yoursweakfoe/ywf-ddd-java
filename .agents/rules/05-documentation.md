...
| 触发条件 | 必须更新的文档 |
|---------|--------------|
| 修改 sample-application 代码结构（新增/删除/重命名文件） | 对应 `docs/sample-application/cookbook/` 文档 |
| 新增 common 模块公开 API | `ywf-ddd-common/docs/common-{module}.md` |
| 设计决策变更（采纳/弃用某模式） | `docs/references.md` |
| 新增聚合 | `docs/sample-application/directory-structure/overview.md` |
| 层间协作关系变化 | 对应 `docs/sample-application/module-design/{layer}.md` |

## 文档位置约定

| 内容类型 | 位置 |
|---------|------|
| 业务应用设计文档 | `docs/sample-application/` |
...
- 新增错误码时必须同步更新 `docs/sample-application/cookbook/error-handling.md` 中的错误码清单
