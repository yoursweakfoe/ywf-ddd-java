...
| 记一个新的设计决策 | 有案卷 → 本案 specify §裁决记录落条；无案卷「修码就法」→ 法卷生效登记行＋theory-map 账本行（→ 归属法 §4）；不设独立判例区 |
...
- 拍板前先翻旧案：查 `specs/archive/` 各封存案卷 specify §裁决记录与 `docs/explanation/`，确认无撞案（**案卷先行**）；撞案 = 新案 §裁决记录宣告 supersede，旧案一字不动。

## 第 3 步：铺四件套

从 `_template/` 复制四文件到 `changes/{slug}/` 填空，一件一身份（形状细则以模板原文为准，不自创变体）：

- `specify.md` —— 做什么与算不算成：Why / What changes / 验收 AC 账（人话摘要）/ 约束 / 不做 / 裁决问句＋**§裁决记录**（裁决唯一居所：门次收据、裁定与当时理由、supersede 宣告；裁一条落一条，落笔不改）；零实现、零法条正文
...
- **每案两停、不论案之大小；法面不设并审或豁免条款**——放行只出自会话中项目主的明示指令（法条刚性、出口在对话 → 案卷 2026-09-sdd-four-stage §裁决记录）。
- 两门收据（门档、日期、项目主原语摘录）均须落本案 specify **§裁决记录**；Q-x 裁定与当时理由同落此节。
...
2. **整目录** `git mv` 到 `archive/`——不拆件、不留守。**清册 bill 例外（自焚）**：执行卷宗清册的 bill 不折叠——末步整删自身案卷目录，三区零残留是唯一合格终态（→ 归属法卷 §4「卷宗清册执行」行；该 bill 亦不落 §裁决记录——执行非裁决事件）
...
4. 连带义务：计数宣称、glossary、docs 同题设计卡若被本次改法波及，同 PR 修齐（冲突裁决法卷赢 → 归属法卷 §1）；**瘦身自查**：入档前逐件裁四件——两门收据与各裁定（含当时理由）全落 specify §裁决记录、条文正文只住 plan、账只住 implement；「今天仍成立」的论证经下行复写解读架，快照留 §裁决记录（→ 归属法卷 §4「案卷折叠入 archive 前」行）
5. **⑨⁺ 论证沉淀**：本案所裁裁决的沉淀论证（现行版）同 PR 复写进 `docs/explanation/` 对应解读篇、篇脚回指 `→ 案卷 <date-slug> §裁决记录（决策快照）`；无同题篇立篇走登记法；事务性决策 theory-map 一行账即止（→ 归属法卷 §4「案卷折叠时（⑨⁺）」行）
...
| 以案小为由并门/跳门 | 法面刚性、出口在对话——豁免只出自会话中项目主明示（案卷 2026-09-sdd-four-stage §裁决记录） |
| 裁决散记在 plan 备注或对话摘要里 | 问与裁一案两节：问题住 specify 前段，裁定唯住 §裁决记录（裁一条落一条，落笔不改） |
