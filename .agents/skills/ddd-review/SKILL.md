...
- `docs/common/common-test.md` §2（ArchUnit 规则编号表 R1-R15 / C1——教义单一事实源，检查项只引编号不复述）
...
- [ ] Domain 零框架**运行时**依赖（违反 = R3/R4/R15）。唯一例外：`org.springframework.stereotype` 装配注解——DomainService 标注 `@Service` 为教义允许（A2 白名单，见 ApplicationArchitectureTest）
...
- [ ] 写端口接口在 `domain/{agg}/repository/domain/`、读端口在 `application/{agg}/repository/application/`；实现分别在 `infrastructure/persistence/{ds}/{agg}/repository/domain|application/`（违反 = R5a/R5b）
- [ ] PO 零 ORM 注解 + XML 七语句契约（schema 前缀 / version 条件 / is_delete 过滤 / existsById 恒返回 boolean）——法条见 rules 04「持久化与 SQL」，详表见 `docs/common/common-ddd.md` §2，模板见 cookbook/new-aggregate.md ⑲；全仓禁 `com.baomidou`（违反 = R15）
...
- [ ] 新增文件位于正确的聚合子包内（必含子段：`handler/command|query/`、`repository/domain/`、`repository/application/`、`adapter/rest/controller/`）
