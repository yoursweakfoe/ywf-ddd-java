...
- [ ] 无具名领域异常类（统一 BusinessException + i18n 位点）
- [ ] 错误码符合 `{aggregate}:err.{scene}` 格式
- [ ] 不变量校验使用显式 `if + throw new BusinessException(key)`
...
- [ ] 未引入当前不使用的组件
- [ ] 无死代码（注释块 / TODO-restore / 空实现）
- [ ] common 模块依赖不超过编译所需的最小集
