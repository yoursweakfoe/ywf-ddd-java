...
5. ㉒ `contract/{agg}/enums/{Agg}Status.java` — 契约枚举（CO 值域镜像 domain ⑬ 状态机；两枚举并存不可合并，奇偶由 ContractEnumParityTest 守卫锁死，见 cookbook ㉒ 小节）

### Phase 2: domain 层（⑫-⑭）

6. ⑫ `domain/{agg}/model/{Agg}.java` — 聚合根（`extends AggregateRoot<UUID>`，纯状态机 + validate，见 cookbook ⑫）
...
9. ⑮ `infrastructure/persistence/master/{agg}/mybatis/po/{Agg}PO.java` — 纯 `@Data` POJO，零 ORM 注解（表名 / 版本条件 / 逻辑删除全在 SQL 文本，见 cookbook ⑮）
10. ⑰ `infrastructure/persistence/master/{agg}/mybatis/mapper/{Agg}Mapper.java` — `@Mapper extends DddMapper<{Agg}PO>`（见 cookbook ⑰）
11. ⑯ `infrastructure/persistence/master/{agg}/converter/{Agg}Converter.java` — `BasicConverter` 桥，`toDomain()` 走 `reconstitute()`（见 cookbook ⑯）
12. ⑲ `src/main/resources/mapper/{agg}/{Agg}Mapper.xml` — 手写 DddMapper 七条语句（法条见 rules 04「持久化与 SQL」，逐条模板见 cookbook ⑲）
13. ⑱ `infrastructure/persistence/master/{agg}/repository/{Agg}RepositoryImpl.java` — 继承 `MybatisPersistence`，构造器注入 Mapper + Converter + `Clock` + `AuditProperties` + `ObjectProvider<CurrentUserProvider>`；不标 `@Transactional`（R11，见 cookbook ⑱）
...
16. ⑨ `application/{agg}/presenter/{Agg}Presenter.java` — DTO → CO（`String → {Agg}Status` 契约枚举在此 `valueOf` 收口，见 cookbook ⑨）
