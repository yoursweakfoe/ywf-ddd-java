...
└── {aggregate}/                        # 按聚合自包含
    ...
    │   └── publisher/                  # Publisher（出站翻译 + 捕获）
    │       └── {Xxx}EventPublisher.java    # 被 CommandHandler/DomainEventListener 调用 → 翻译为集成事件 → 同事务捕获入集成 Outbox
...
| `{aggregate}/event/publisher/` | 被 CommandHandler/DomainEventListener 显式调用 → 翻译为契约 IntegrationEvent → 经 `IntegrationEventOutboxStore` 同事务捕获入集成 Outbox（投递 MQ 归框架排空器）。AppService 不直接依赖 publisher |
