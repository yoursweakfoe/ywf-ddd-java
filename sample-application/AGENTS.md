...
- **契约先行（spec-first 的落地形态）**：新增/变更行为先立 `specs/changes/<YYYY-MM-slug>/` 四件套（本树，模板与框架区共用：specify→plan→tasks→implement）；实现完成后 delta 归档折叠进本树 `specs/current/`。文档同步义务只在归档那一刻发生。
