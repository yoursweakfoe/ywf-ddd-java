 ...
 * 铸造策略是框架装配宣言的一部分，业务侧不裸 import JUG），再经 {@link ProductId#of}
 * 装箱为专属币种（法卷锚 BP-13，案卷 2026-09-typed-identifier）——
...
public class ProductFactory implements Factory {
    ...
    public Product create(String name, BigDecimal price, int stock) {
        var product = new Product(ProductId.of(AggregateIds.mint()), name, price, stock);
        product.validate();
        return product;
    }
}
