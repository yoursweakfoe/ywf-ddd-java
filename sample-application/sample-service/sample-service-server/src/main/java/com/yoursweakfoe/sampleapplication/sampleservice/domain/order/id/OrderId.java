...
/**
 * 订单聚合身份终类型 —— {@code Order} 的专属币种（聚合根身份槽 / 写端口 ID 槽实参）。
 *
 * <p>只装箱不站岗：紧凑构造器仅 null 检查，不校验底值格式、不校验 UUID 版本位
 * （装载存量行照过 {@link #of(UUID)}，出生保证归铸造入口 {@code OrderFactory}）。
 ...
 */
public record OrderId(UUID value) implements Identifier<UUID>, Serializable {

    public OrderId {
        if (value == null) {
            throw new BusinessException("order:err.idRequired");
        }
    }
